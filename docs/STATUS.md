...
⚠ **XIAO 는 아직 수신이 확인되지 않았다.** 같은 코드로 NU54-DK 는 되는데
XIAO 는 0바이트다. 세션 초반에 한 번은 됐던 적이 있어 핀 정의(`P1.08`/`P1.09`)
문제로 보이지는 않는다. 온보드 CMSIS-DAP 의 VCOM 이 여러 번 리셋 뒤 멎었을
가능성이 있어 **USB 를 뽑았다 꽂고 다시 확인해야 한다.**
