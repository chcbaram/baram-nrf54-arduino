...
릴리스는 **`0.4.0`** 이 최신이다 (2026-09-14, XIAO nRF54LM20A 추가 — §2.17).
다음 릴리스 절차는 §(c).
...
#### 크기 — 처음 잰 값이 틀렸다 (2026-09-12 정정)

처음에 "코어에 두어도 크기가 늘지 않는다, blink 가 25,264 B 로 전후 동일" 이라고
적었다. **기준선이 오염된 측정이었다.** `cores/` 아래는 무조건 컴파일되므로
(§7 F13 ②) `wiring_interrupt.c` 를 만들어 둔 뒤에 "붙이기 전" 을 쟀고,
그 뒤 고친 `nrfx_config.h` / `Arduino.h` 는 링크에 영향이 없어 앞뒤가 같게 나왔다.

**파일을 실제로 치우고 다시 재야 한다.**

| blink | flash |
|---|---|
| 진짜 기준선 | **23,696 B** |
| 벡터 → 드라이버 직접 호출 (GPIOTE) | 25,264 B (**+1,568**) |
| 거기에 PWM 까지 | 25,840 B (**+2,144**) |
| **함수 포인터 트램폴린 (현재)** | **23,784 B** (**+88**) |

**원인**: 벡터 테이블은 링커가 `KEEP` 한다. 벡터가 드라이버 핸들러를 직접 부르면
`--gc-sections` 이 그 드라이버를 영영 못 걷어낸다.

**해결**: 포인터를 한 겹 둔다. 벡터는 트램폴린만 붙잡고, 진짜 핸들러는 초기화
경로에서만 참조되므로 안 쓰면 GC 된다. `wiring_private.h` 에 패턴을 적어 두었다.

앞서 적은 일반화 *"함수만 두면 부르는 스케치만 치른다"* 도 **틀렸다.**
맞는 규칙은 이것이다:

> **`--gc-sections` 의 루트는 `KEEP` 된 섹션이다** — 벡터 테이블과 `.init_array`.
> 거기서 도달 가능한 것은 전부 모든 스케치에 들어간다. 코어에 페리페럴을 붙일 때는
> **벡터에서 드라이버까지의 경로를 끊어라.**
...
### XIAO nRF54LM20A 추가 (2026-09-12)

**variant 가 없는 보드도 표는 낼 수 있다.** `docs/boards/` 와 Pin Planner 만으로
만들어지므로, M6 에서 variant 를 쓰기 전에 먼저 핀을 검토할 수 있다
(그럴 때 'Name in sketch' 열은 통째로 빠진다).

패키지를 **회로도의 GPIO 개수로 확정**했다 — P0=10 / P1=32 / P2=11 / P3=13, 합계 66.
Pin Planner 의 LM20A 세 패키지 중 **FCCSP98 만** 이 수와 맞는다 (CSP61 은 40,
QFN52 는 32이고 **P3 가 아예 없다**). 회로도·Zephyr 보드 정의·Pin Planner 셋이
D0~D27 · LED · 버튼 · AIN 배정까지 전부 일치했다.

#### 이 보드가 알려 준 것

**1. LM20A 에서는 도메인 20 이 포트를 둘 소유한다 — P1 과 P3.**
CLAUDE.md §4.2 가 "규칙이 단순 확장되지 않을 수 있다" 며 열어 둔 항목이었다.
SPIM/TWIM/UARTE 20~24, PWM20~22, GPIOTE20 이 양쪽에 닿고, SAADC·PDM·TDM·QDEC 는
P1 전용이다. **SERIAL 23·24 가 GRTC 와 같은 0x500E 대역에 있는데도 GPIO 는
도메인 20 쪽**이라, 주소 대역으로 도메인을 추측하면 안 된다는 것도 확인됐다.

**2. 실크스크린의 `A8` 은 아날로그 입력이 아니다.** 넷 이름이 `P1.07/SCL/A8/D5`
라 A8 로 보이지만 P1.07 에 배정된 것은 `SAADC.EXTREF` 다. 표에 `ADC` 열이 있어
`x` 로 바로 드러난다 — 열을 넣지 않았으면 못 봤을 것이다.

**3. XIAO nRF54L15 와 반대로 LED 세 개가 전부 P1 이다.** 즉 `analogWrite` 가 된다.
L15 판은 LED 가 P2.00 하나뿐이라 PWM 도 인터럽트도 안 됐다.

⚠ 확정 못 한 것 둘: **헤더의 물리 핀 번호**(회로도 커넥터 심볼이 15~23번을 이름
없이 둔다 — 표는 D 번호를 축으로 쓴다)와 **QSPI 플래시의 핀별 결속**(여섯 핀이
쓰인다는 것은 확실하나 어느 넷이 어느 볼인지는 도면을 눈으로 봐야 한다).
둘 다 보드 문서에 적어 두었고 M6 항목이다.
...
## 2.15 `analogWrite` (PWM) — ✅ (2026-09-12)

`cores/nrf54l/wiring_analog.{h,c}`. 실기 기록은 `docs/HIL/M2-pwm.md`.

PWM20/21/22 각 4채널 = **동시에 12핀**, 전부 도메인 20 이라 **P1 전용**이다
(LM20A 는 P3 도). `analogWriteOk()` 로 실패를 알 수 있고,
`analogWriteResolution(1~15)` 로 해상도를 바꾼다 — 기준 클럭이 1 MHz 고정이라
**주파수가 같이 바뀐다** (8비트 3.9 kHz, 12비트 244 Hz).

⚠ **NU54-DK 의 `LED_BUILTIN`(P2.09)에는 안 된다.** LED2(P1.10)/LED4(P1.14) 를 써라.
XIAO nRF54L15 는 사용자 LED 가 P2.00 하나뿐이라 아예 안 된다.
PinMap 표가 미리 알려 준 덕에 시험 스케치를 처음부터 맞게 짰다.

#### 듀티를 `digitalRead` 로 재면 안 된다

첫 시험 스케치가 그랬는데 **항상 0 % 가 나온다.** `digitalRead()` 는 출력 핀에서
**OUT 레지스터**를 읽는데 **PWM 은 OUT 을 거치지 않고 PSEL 로 패드를 직접 몬다.**

→ 입력 버퍼를 다시 붙이고 IN 레지스터를 직접 읽었다. 출력 방향은 유지하므로
구동을 방해하지 않는다. `analogWrite()` 가 핀을 다시 잡을 때마다 떨어지므로
매번 다시 붙여야 한다.

측정: 요청 0/25/50/74/100 % → **0/24/50/74/100 %**.
**극성이 뒤집혔으면 100/76/50/26/0 이 나왔을 것이다** — LED 밝기만으로는
못 봤을 값이다.

#### 극성 비트가 직관과 반대다

듀티 값의 MSB(15)가 극성이고 **정상(액티브 하이)일 때 그 비트를 세운다.**
근거는 Zephyr `drivers/pwm/pwm_nrfx.c` 의
`(compare_value | (inverted ? 0 : PWM_NRFX_CH_POLARITY_MASK))`.
100 % 는 `top` 이 아니라 `0x7FFF` 을 준다 — `top` 이면 한 틱 모자라 99.6 % 다.

#### 인터럽트를 안 써도 벡터는 이어야 한다
