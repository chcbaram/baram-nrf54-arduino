...
| 큐 깊이 | 필요한 앱 RAM 시작 | 증가분 | 경계 `0x20007800` |
|---|---|---|---|
| **1** | `0x20007590` | — | ✅ |
...
**그래서 링크 수와 큐 깊이는 맞바꾸는 관계다.** 같은 30 KB 예약 안에서:

| 원하는 것 | 조합 | 결과 |
|---|---|---|
| 연결 많이 | 링크 5 · 큐 1 | ✅ 여유 624 B — **큐를 못 올린다** |
| 처리량 | 링크 4 · 큐 3 | ✅ 여유 약 2.6 KB (Adafruit `BANDWIDTH_MAX` 수준) |

⚠ 기본값은 **1 로 둔다** (Adafruit 기본과 같다). 어느 쪽을 원하는지는 스케치가 아는
것이지 코어가 정할 일이 아니다. 지금은 `boards.txt` 의 `build.extra_flags` 에
`-DSD_BLE_HVN_TX_QUEUE_SIZE=N` 으로 바꾼다. 상류처럼 스케치에서 고르게 하려면
