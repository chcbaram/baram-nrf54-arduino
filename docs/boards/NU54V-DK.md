...
PMIC 의 나머지 신호는 nINT / nPG / nCE / VBAT_MON 이다.
**어느 브리지가 어느 신호인지는 아직 확정하지 못했다.**
`VBAT_MON` 은 아날로그이므로 **A4 / A5 중 하나일 가능성이 높다** — 그렇다면
`analogRead` 로 배터리 전압을 직접 읽을 수 있다. 확인하면 여기에 적을 것.

### 예제

`libraries/NU54V-DK/examples/pmic` — 충전 상태·목표 전압·입력 전원 유효 여부를
읽는다. 실기 출력:

```
MASK_ID 0x41
charge: constant voltage   VIN good
  target 4.20 V   STAT0 0x41  STAT1 0x00  ICHG 0x05
```

**읽기만 한다.** 충전 설정을 잘못 쓰면 충전이 멎거나 그 이상이 될 수 있다.
...
| A4 | P1.11 | PMIC 제어/측정 | **SB1 (붙어 있음)** |
| A5 | P1.12 | PMIC 제어/측정 | **SB4 (붙어 있음)** |
| A6 | P1.13 | SW1 | — |
| A7 | P1.14 | LED4 | — |
...
| 핀 | 이름 | GPIO | | 핀 | 이름 | GPIO |
|---|---|---|---|---|---|---|
| 9 | A3 / UART2 CTS | P1.07 | | 20 | — | P2.06 |
| 10 | A2 / UART2 RTS | P1.06 | | 21 | LED3 / SWO | P2.07 |
| 11 | A1 / UART2 RX | P1.05 | | 22 | PMIC | P2.08 |
| 12 | A0 / UART2 TX | P1.04 | | 23 | LED1 | P2.09 |
| 16 | SW3 | P1.08 | | 24 | PMIC | P2.10 |
| 17 | — | P2.04 | | 25 | Serial TX | P0.00 |
| 19 | — | P2.05 | | 26 | Serial RX | P0.01 |

표에 없는 번호는 GND / VDD_MOD 다. 27 `SWDCLK` · 28 `SWDIO`.
...
| 8 | LED2 | P1.10 | | 20 | — | P2.01 |
| 9 | A4 / PMIC | P1.11 | | 21 | — | P2.02 |
| 10 | A5 / PMIC | P1.12 | | 22 | — | P2.03 |
| 11 | SW1 / A6 | P1.13 | | | | |
...
전원: 25 `VDD_MOD` · 26 `VDD_3V3_SYS`(3.3V 출력) · 27 `VBAT` ·
29 `VEXT` · 30 `VBUS`(5~14V 입력).
...
- [ ] 헤더 핀 번호를 실물 실크스크린과 대조
- [x] `SB5`~`SB8` — `Serial` 이 동작하므로 붙어 있다

- [ ] `SB1`~`SB4` 중 어느 것이 nINT / nPG / nCE / VBAT_MON 인지

**확인된 것**: `SB1`~`SB4` 붙어 있음, `SB20`/`SB21` 미실장(크리스털 고정),
