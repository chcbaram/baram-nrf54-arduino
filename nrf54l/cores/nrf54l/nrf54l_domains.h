 ...
 *
 * ⚠ **이 헤더는 포트까지만 본다.** 그것만으로는 부족한 경우가 있다 —
 *   P2.03 은 P2 가 맞지만 SPIM00.SCK 로는 못 쓴다 (SCK 는 P2.01·P2.06 뿐).
 *   **핀 번호까지 보려면 `nrf54l_pinmap.h` 의 `NRF54L_ASSERT_SIG` 를 써라.**
 *   variant 들은 이미 그쪽으로 옮겼고, 여기 매크로들은 그 표에 없는
 *   페리페럴(예: TIMER, COMP)을 검증할 때 남겨 둔다.
 */
 ...
 *   2. **이 매크로는 포트까지만 본다.** P2 의 어느 핀이 되는지는 이제
 *      확인됐으므로(docs/PERIPHERAL-PINMAP.md §4), 새 코드는
 *      `nrf54l_pinmap.h` 의 `NRF54L_ASSERT_SIG(pin, UARTE21_TXD, ...)` 를 써라.
 *      그쪽이 핀 번호까지 본다
 *
