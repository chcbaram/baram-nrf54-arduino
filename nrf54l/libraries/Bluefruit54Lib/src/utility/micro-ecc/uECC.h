...
/* Optimization level; trade speed for code size.
   ...
   curve is enabled. */
/* ─────────────────────────────────────────────────────────────────────
 * baram-nrf54l-arduino 로컬 설정 (vendoring 시 추가한 부분)
 *
 * 원본은 모든 커브를 켜고 플랫폼을 컴파일러 매크로로 추측한다. 우리는
 * **LESC 에 쓰는 P-256(secp256r1) 하나만** 필요하고, 나머지를 켜 두면
 * 플래시만 먹는다. 상세는 docs/PATCHES.md.
 *
 * ⚠ uECC_VLI_NATIVE_LITTLE_ENDIAN 은 **건드리지 않는다.** Nordic 문서가
 *   "바꾸면 라이브러리가 제대로 동작하지 않을 수 있다" 고 경고한다.
 *   BLE 는 키를 리틀엔디안으로 주고받으므로, 엔디안 변환은 우리 쪽
 *   BLESecurity 에서 명시적으로 한다.
 * ───────────────────────────────────────────────────────────────────── */
